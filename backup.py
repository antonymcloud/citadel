...
from models import db, Repository, Job
...
def create_backup():
    if request.method == 'POST':
        ...
        if not repo_id or not source_path:
            flash('Repository and source path are required.', 'danger')
            repos = Repository.query.filter_by(user_id=current_user.id).all()
            return render_template('create_backup.html', repos=repos)
...
@login_required
def list_jobs():
    # Exclude 'list' jobs from the job history
    jobs = Job.query.filter_by(user_id=current_user.id).filter(Job.job_type != 'list').order_by(Job.timestamp.desc()).all()
